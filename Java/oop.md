...
* Java에서 추상화는 주로 추상 클래스와 인터페이스를 통해 이루어진다.
...
* Java는 순수 추상 클래스를 더 편리하게 사용할 수 있도록 ``인터페이스``라는 개념을 제공한다.
...
* 클래스, 추상 클래스, 인터페이스는 프로그램 코드, 메모리 구조상 모두 똑같다. 모두 Java에서는 .class 로 다루어진다. 인터페이스를 작성할 때도 .java 에 인터페이스를 정의한다.
...
* 다중 구현 : Java에서 클래스 상속은 부모를 하나만 지정할 수 있다. 반면에, 인터페이스는 부모를 여러명 두는 다중 구현(다중 상속)이 가능하다.
